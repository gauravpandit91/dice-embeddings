...
class DistMult(BaseKGE):
    ...
    def forward_triples(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # (2) Compute the score
        u = (self.hidden_dropout(self.hidden_normalizer(head_ent_emb * rel_ent_emb)) * tail_ent_emb).sum(dim=1)
        return u
...
class distMult(BaseKGE):
    ...
    def forward_triples(self, x: torch.Tensor) -> torch.Tensor: # idx_triplet = (h_idx, r_idx, t_idx)
        head_ent_emb, rel_ent_emb, tail_ent_emb = self.get_triple_representation(x)
        # h_embedding = self.entity_embeddings(head_ent_emb)
        # r_embedding = self.relation_embeddings(rel_ent_emb)
        # t_embedding = self.entity_embeddings(tail_ent_emb)
        score = (((head_ent_emb * rel_ent_emb)) * tail_ent_emb).sum(dim=1)#(head_ent_emb * rel_ent_emb * tail_ent_emb).sum()
        return score#torch.sigmoid(dist).unsqueeze(0) #just to be sure values are in [0 1]
